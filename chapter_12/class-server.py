...
'''
Like the manually-coded servers we’ve just studied, this module’s primary classes implement
servers which process clients in parallel (a.k.a. asynchronously) to avoid denying
service to new requests during long-running transactions. Their net effect is to
...
'''
focuses on service implementation (the customized handle
method), not on threading details.
'''


"""
To build a forking server instead, just use the class name ForkingTCPServer when creating
the server object.
"""
