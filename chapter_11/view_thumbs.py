...
def makeThumbs(imgdir, size=(150, 150), subdir='thumbs'):
    """
...
def viewer(imgdir, kind=Toplevel, cols=None):
    ...
    """
    win = kind()
    win.title('bimri Viewer: ' + imgdir)
    quit = Button(win, text='QUIT', command=win.quit, bg='beige')   # pack fist
    quit.pack(fill=X, side=BOTTOM)
    thumbs = makeThumbs(imgdir)
    if not cols:
        cols = int(math.ceil(math.sqrt(len(thumbs))))       # fixed for N x N
...
if __name__ == '__main__':
    imgdir = (len(sys.argv) > 1 and sys.argv[1]) or r'C:\Users\avatar\Pictures\gifs'
