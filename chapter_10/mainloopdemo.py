...
class Demo(Frame):
    ...
    def savefile(self):
        self.save_name = asksaveasfilename(initialdir='C:\\Python38')
...
'''
Notice how this program calls mainloop twice, to implement two modal user interactions
from an otherwise non-GUI script. It’s OK to call mainloop more than once, but
this script takes care to re-create the GUI’s widgets before each call because they are
destroyed when the previous mainloop call exits
